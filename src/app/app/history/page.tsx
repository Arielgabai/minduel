...
export default async function HistoryPage() {
  ...
  // Progression par compétence (moyenne sur les 2 dernières vs 2 premières).
  ...
  return (
    <div className="animate-fade-up">
      <h1 className="mb-4 text-2xl font-bold">Historique</h1>
              ...
              <Link key={s.id} href={`/app/analysis/${s.id}`}>
                <Card hover className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-white">{s.scenario.name}</p>
              ...
              <Card>
                <div className="space-y-3">
                  {[...bySkill.entries()].map(([label, agg]) => {
                    const pct = agg.max > 0 ? Math.round((agg.total / agg.max) * 100) : 0;
                    return (
                      <div key={label}>
